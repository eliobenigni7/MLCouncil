...
- Narrow interval → high confidence  → multiplier close to 1.8
- Wide interval   → low confidence   → multiplier close to 0.3

Usage
-----
>>> sizer = ConformalPositionSizer(coverage=0.80)
...
class ConformalPositionSizer:
    ...
    coverage:
        ...
        Default 0.80 → 80 % prediction intervals.  Reduced from 0.85 to
        tighten intervals and increase average position multipliers by ~15 %,
        improving expected alpha capture.  The 20 % miss rate is acceptable
        for daily cross-sectional signals where diversification across the
        investable universe limits tail exposure from individual misses.
    """
    ...
    def compute_position_multipliers(
        ...
        3. multiplier  = exp(1 - width_norm), clipped to [0.3, 1.8].

        Stocks with narrow intervals (high confidence) receive multipliers
        near 1.8; stocks with wide intervals receive multipliers near 0.3.
        ...
        Returns
        -------
        pd.Series(index=ticker, values=float) — multipliers in [0.3, 1.8].
        """
    ...
    def filter_low_confidence(
        ...
        threshold_percentile:
            ...
            are set to 0.  Default 85 → drop the widest-interval 15 %.
            Lowered from 90 to remove more low-confidence signals while
            preserving most of the investable universe.

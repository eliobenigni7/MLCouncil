...
FastAPI :8000  ── routers ────────────────────────────────────
  ...
  pipeline  portfolio  config  monitoring  trading  intraday  │
        │ services: experiment_service (thread pool + job    │
        │ registry on data/results/experiments/),            │
...
8. **Milestone before retirement**: the SPA runs alongside the old UIs on :8000 during development (no shippable-gap: old UIs stay live until the checklist is green); retirement of Streamlit/admin SPA happens in the same release only after items 1–7 pass.
